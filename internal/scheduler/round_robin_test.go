...
func TestRoundRobinRotates(t *testing.T) {
	...
	for i, expected := range want {
		got, err := rr.Schedule(context.Background(), model.AllocationRequest{}, NewStaticCandidates(servers))
		if err != nil {
...
func TestRoundRobinFiltersUnavailableAndSemanticConstraints(t *testing.T) {
	...
	got, err := rr.Schedule(context.Background(), model.AllocationRequest{Region: "sg", Version: "v1"}, NewStaticCandidates(servers))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRoundRobinNoCandidate(t *testing.T) {
	rr := NewRoundRobin()
	_, err := rr.Schedule(context.Background(), model.AllocationRequest{}, NewStaticCandidates([]model.GameServerSnapshot{{ID: "x", State: model.ServerDraining, Capacity: 10}}))
